...
renderText({
  ...
  if (tc_regression_r_squared == 1) {
    out <- paste("WARNING: The regression line has a perfect correlation. This is possible, but surprising. Did you measure on three separate breaths?")
  }

  if (tc_regression_r_squared < 1 & tc_regression_r_squared > 0.8) {
    out <- paste("The regression r^2 value is: ", round(tc_regression_r_squared, 2), ". This is likely acceptable.", sep = "")
  }
  if (tc_regression_r_squared < 0.8) {
    out <- paste("WARNING: The regression r^2 value is: ", round(tc_regression_r_squared, 2), ". This suggests a variable expiratory flow waveform. Possible explantions include secretions, active exhalation, or dynamic airway collapse.", sep = "")
  }
  out
})
```
