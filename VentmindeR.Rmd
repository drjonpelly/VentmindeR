...
This tool was built using [RStudio](https://rstudio.com/products/rstudio/) running `r sessionInfo()$R.version$version.string`, and the following packages:
...
renderText({
  ...
  paste("The patient's time constant is: ", round(time_constant, 2), " seconds. The patient must have an exhalation time of ", round(time_constant * 3, 2), " seconds. Given that you have specified an inspiratory time of ", input$tc_itime, " seconds, the maximum safe respiratory rate for this patient is ", round(60 / (time_constant * 3 + input$tc_itime), 1), " breaths per minute to prevent air trapping.", sep = "")
})
```
