...
output$rate_numeric <- renderText({
  ...
  out_negative_value_warning <- ifelse(input$current_pco2 < 0 | input$current_rate < 0 | input$desired_pco2 < 0,
    paste("<br><br><span style=\"color:red\">WARNING: you entered at least one negative value. This is not possible, and will generate an unusable result.</span>"),
    paste("")
  )
  return(paste(out_result, out_rate_limit_warning, out_integer_warning, out_co2_integer_warning, out_negative_value_warning))
})
...
output$rv_numeric <- renderText({
  ...
  warning_negative_value <- ifelse(input$rv_current_pco2 < 0 |
  ...
  return(paste(rv_result, warning_vt, warning_rate, warning_integer, warning_negative_value))
})
...
output$warning_compliance <- renderText({
  warning_compliance_height <- case_when(
    input$cr_height > 200 ~ paste("<span style=\"color:red\"><br>WARNING: you chose a height of ", input$cr_height, " cm. The average height of adult males and females are 175 cm and 162 cm, respectively. Is this deliberate?</span>"),
    input$cr_height < 40 ~ paste("<span style=\"color:red\"><br>WARNING: you chose a height of ", input$cr_height, " cm. The average height of a term newborn is 50 cm. Is this deliberate?</span>"),
    TRUE ~ paste("")
  )
  warning_compliance_integer <- ifelse(is.integer(input$cr_vt) & is.integer(input$cr_pip) & is.integer(input$cr_pplat) & is.integer(input$cr_peep),
    paste(""),
    paste("<br><span style=\"color:red\">WARNING: you have entered non-integer values other than inspiratory time or height. Is this deliberate?</span>")
  )
  warning_negative_value <- ifelse(input$cr_vt < 0 |
    input$cr_pip < 0 |
    input$cr_pplat < 0 |
    input$cr_peep < 0 |
    input$cr_itime < 0,
  paste("<br><span style=\"color:red\">WARNING: you entered at least one negative value. This is not possible, and will generate an unusable result.</span>"),
  paste("")
  )
  warning_illogical_pressures <- case_when(
    input$cr_pip < input$cr_pplat ~ "<br><span style=\"color:red\">WARNING: you entered a PIP less than plateau pressure. This is only possible in a patient with active inhalation. Please re-check your measurements.</span>",
    input$cr_pplat <= input$cr_peep ~ "<br><span style=\"color:red\">WARNING: you entered a plateau pressure less than or equal to PEEP. Is your patient in CPAP? Please do not use this value, and re-check your measurements.</span>",
    input$cr_pip <= input$cr_peep ~ "<br><span style=\"color:red\">WARNING: you entered a PIP less than or equal to PEEP. Is this intentonal? If so, it may imply that your patient is actively inhaling, and not receiving adequate ventilator support.</span>",
    TRUE ~ paste("")
  )
  return(paste(warning_compliance_height, warning_compliance_integer, warning_negative_value, warning_illogical_pressures))
})
...
output$tc_warning <- renderText({
  ...
  if (tc_regression_r_squared < 0.8) {
    out <- paste("<br><span style=\"color:red\">WARNING: the regression adjusted r^2 value is: ", round(tc_regression_r_squared, 3), ". This suggests a variable expiratory flow waveform. Possible explantions include secretions, active exhalation, or dynamic airway collapse.</span>", sep = "")
  }

  warning_negative_value <- ifelse(input$tc_v_1 < 0 |
    input$tc_flow_1 < 0 |
    input$tc_v_2 < 0 |
    input$tc_flow_2 < 0 |
    input$tc_v_3 < 0 |
  ...
  return(paste(out, warning_negative_value))
})
...
output$vd_vt <- renderText({
  ...
  warning_negative_value <- ifelse(input$vdvt_paco2 < 0 |
    input$vdvt_peco2 < 0,
  paste("<br><br><span style=\"color:red\">WARNING: you entered at least one negative value. This is not possible, and will generate an unusable result.</span>"),
  paste("")
  )
  warning_negative_vd_vt <- ifelse((input$vdvt_paco2 - input$vdvt_peco2) / input$vdvt_paco2 < 0,
    paste("<br><br><span style=\"color:red\">WARNING: the calculated Vd/Vt is negative. While this is possible in special circumstances (Karnik et. al.), you should be skeptical of the result. Did you reverse PaCO2 and End Tidal CO2? </span>"),
    paste("")
  )
  return(paste(vd_vt_numeric, warning_negative_value, warning_negative_vd_vt))
})
...
output$pf_oi_calculation <- renderText({
  ...
  numeric_calculation <- case_when(
    ...
    input$oi_desired == "S/F and OSI" & input$oi_pao2 > 100 ~ paste("<span style=\"color:red\">WARNING: You have selected an oxygen saturation of >100%. This is not posible. Did you mean to enter a PaO2?</span>"),
    input$oi_desired == "S/F and OSI" & input$oi_pao2 == 100 ~ paste("<span style=\"color:red\">WARNING: You have selected an oxygen saturation of 100%. Is this actually your patient's oxygen saturation? If so, you should not attempt to calculate an an S/F ratio or OSI. It is impossible to infer a PaO2 from an SpO2 of 100% (e.g. both a PaO2 of 150 mmHg and 450 mmHg will result in an SpO2 of 100%). Did you mean to enter a PaO2?</span>"),
...
output$qp_qs_result <- renderText({
  ...
  pv_sat_warning <- case_when(
    input$pv_sat > 100 ~ paste("<br><br><span style=\"color:red\">WARNING: you have entered a pulmonary venous oxygen saturation greater than 100%. This is not physiologically possible."),
  ...
  ao_sat_warning <- case_when(
    input$ao_sat > 100 ~ paste("<br><br><span style=\"color:red\">WARNING: you have entered an aortic oxygen saturation greater than 100%. This is not physiologically possible."),
  ...
  svc_sat_warning <- case_when(
    input$svc_sat > 80 ~ paste("<br><br><span style=\"color:red\">WARNING: you have entered a mixed venous aortic oxygen saturation greater than 80%. Is this deliberate?"),
  ...
  hgb_warning <- case_when(
    input$hgb > 20 ~ paste("<br><br><span style=\"color:red\">WARNING: you have entered a hemoglobin greater than 20 mg/dl. Is this deliberate?"),
    input$hgb < 5 ~ paste("<br><br><span style=\"color:red\">WARNING: you have entered hemoglobin less than 5 mg/dl. Is this deliberate?"),
    TRUE ~ paste("")
  )
  pao2_warning <- case_when(
    input$qp_qs_pao2 > 300 ~ paste("<br><br><span style=\"color:red\">WARNING: you have entered a PaO2 greater than 300 mmHg. Is this deliberate?"),
