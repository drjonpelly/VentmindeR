...
renderText({
  ifelse(input$current_pco2 * input$current_rate / input$desired_pco2 >= 28,
    paste("Your desired rate is: ", round(input$current_pco2 * input$current_rate / input$desired_pco2, 0), " breaths/min. WARNING: at this rate, a time constant calculation is recommended to prevent air trapping.", sep = ""),
    paste("Your desired rate is: ", round(input$current_pco2 * input$current_rate / input$desired_pco2, 0), " breaths/min.", sep = "")
  )
})
```
...
numericInput(
  ...
  label = "Desired Tidal Volume (ml/kg) or Rate (breaths/min):",
  value = 8,
  min = 0,
  max = 100,
  step = 1
...
renderText({
  if (input$rv_desired_set == "Set Tidal Volume") {
    ...
    out <- ifelse(desired_rate_rv >= 28,
      paste("Your desired rate is: ", round(desired_rate_rv, 0), " breaths/min. WARNING: at this rate, a time constant calculation is recommended to prevent air trapping.", sep = ""),
      paste("Your desired rate is: ", round(desired_rate_rv, 0), " breaths/min.", sep = "")
    )
  }
...
renderTable({
  ...
  tribble(
    ...
    "Compliance", round(compliance,3), "l/cmH2O",
    "Resistance", round(resistance, 2), "cmH2O*seconds*l",
    "Inspiratory Time Constant", round(time_constant, 2), "seconds",
    "Minimum Safe Exhalation Time", round(time_constant*3, 2), "seconds",
    "Inspiratory Time", input$cr_itime, "seconds",
    "Breath Cycle Time", round(input$cr_itime+(time_constant*3),2), "seconds",
    "Maximum Safe Respiratory Rate", round(60 / (time_constant * 3 + input$cr_itime), 2), "breaths/min"
  )
})
```
...
Below is the patient's calculated _expiratory_ time constant and maximum safe respiratory rate to prevent air trapping. **Please note** that many ventilators round the values on the screen to the nearest ml (for volume) and l/min (for flow). Because this may result in substantial rounding error, it is recommended to perform this calculation at least 3 times.
...
renderTable({
  ...
  tribble(
    ~Variable, ~Value, ~Units,
    "Expiratory Time Constant", round(time_constant, 2), "seconds",
    "Minimum Safe Exhalation Time", round(time_constant*3, 2), "seconds",
    "Inspiratory Time", input$tc_itime, "seconds",
    "Breath Cycle Time", round(input$tc_itime+(time_constant*3),2), "seconds",
    "Maximum Safe Respiratory Rate", round(60 / (time_constant * 3 + input$tc_itime), 2), "breaths/min"
  )
})
```
...
### ARDSnet (Adult) PEEP Recommendations
...
renderText({
  ...
  paste("The pulmonary to systemic blood flow ratio (Qp:Qs) is ", round(qp_qs, 2), " to 1. ", "The patient's blood oxygen content is ", round(carrying_capacity, 2), " ml/dL.", sep = "")
})
```
