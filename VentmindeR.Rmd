...
Medicine and modeling are team sports. The authors wish to acknowledge that the whole product is always greater than the sum of the parts.

[Jonathan Pelletier](https://twitter.com/drjonpelly) is a Pediatric Critical Care Medicine Fellow and an NIH T32 Post-Doctoral Scholar at the University of Pittsburgh His research interests include variability in clinical care, data science, and using machine learning to assist clinical decision making.

[Neil Munjal](https://twitter.com/NeilMunjal) is a Pediatric Critical Care Medicine Fellow, a pediatric neurologist, and an NIH T32 Post-Doctoral Scholar at the University of Pittsburgh. His research interests include multi-modal real-time ICU monitoring, machine learning in medicine, causal modeling, and predictive analytics.

[Jay Rakkar](https://twitter.com/JayRakkar) is a Pediatric Critical Care Medicine Fellow and an NIH T32 Post-Doctoral Scholar at the University of Pittsburgh. His research interests include pediatric brain injury and utilization of big data for clinical research, and developing electronic applications to optimize medical education and patient safety.
...
observeEvent(input$current_pco2, {
  if (!is.numeric(input$current_pco2)) {
    shinyFeedback::feedbackDanger(
      "current_pco2",
      !is.numeric(input$current_pco2),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$current_pco2 <= 0) {
    shinyFeedback::feedbackDanger(
      "current_pco2",
      input$current_pco2 <= 0,
      "This calculator does not support negative values.",
      icon = NULL
    )
  }
  else if (input$current_pco2 < 20 | input$current_pco2 > 60) {
    shinyFeedback::feedbackDanger(
      "current_pco2",
      input$current_pco2 < 20 | input$current_pco2 > 60,
      "This is a potentially dangerous CO2 value. Are you sure?",
      icon = NULL
    )
  }
  else if (!is.integer(input$current_pco2)) {
    shinyFeedback::feedbackWarning(
      "current_pco2",
      !is.integer(input$current_pco2),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
  }
  else {
    shinyFeedback::hideFeedback("current_pco2")
  }
})
...
observeEvent(input$current_rate, {
  if (!is.numeric(input$current_rate)) {
    shinyFeedback::feedbackDanger(
      "current_rate",
      !is.numeric(input$current_rate),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$current_rate <= 0) {
    shinyFeedback::feedbackDanger(
      "current_rate",
      input$current_rate <= 0,
      "This calculator does not support negative values.",
      icon = NULL
    )
  }
  else if (input$current_rate < 10 | input$current_rate > 40) {
    shinyFeedback::feedbackDanger(
      "current_rate",
      input$current_rate < 10 | input$current_rate > 40,
      "This is a potentially dangerous rate. Are you sure?",
      icon = NULL
    )
  }
  else if (!is.integer(input$current_rate)) {
    shinyFeedback::feedbackWarning(
      "current_rate",
      !is.integer(input$current_rate),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("current_rate")
  }
})
...
observeEvent(input$desired_pco2, {
  if (!is.numeric(input$desired_pco2)) {
    shinyFeedback::feedbackDanger(
      "desired_pco2",
      !is.numeric(input$desired_pco2),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$desired_pco2 <= 0) {
    shinyFeedback::feedbackDanger(
      "desired_pco2",
      input$desired_pco2 <= 0,
      "This calculator does not support negative values.",
      icon = NULL
    )
  }
  else if (input$desired_pco2 < 20 | input$desired_pco2 > 60) {
    shinyFeedback::feedbackDanger(
      "desired_pco2",
      input$desired_pco2 < 20 | input$desired_pco2 > 60,
      "This is a potentially dangerous CO2 value. Are you sure?",
      icon = NULL
    )
  }
  else if (!is.integer(input$desired_pco2)) {
    shinyFeedback::feedbackWarning(
      "desired_pco2",
    ...
    shinyFeedback::hideFeedback("desired_pco2")
  }
})
```
...
rate_calc_desired_rate <- reactive({
  validate(
    need(input$current_pco2 > 0, "This calculator only accepts values greater than zero. Please enter a current pCO2 value greater than zero."),
    need(input$current_rate > 0, "This calculator only accepts values greater than zero. Please enter a current rate value greater than zero."),
    need(input$desired_pco2 > 0, "This calculator only accepts values greater than zero. Please enter a desired pCO2 value greater than zero.")
  )
  round(input$current_pco2 * input$current_rate / input$desired_pco2, 0)
})

output$rate_numeric <- renderText({
  out_result <- paste("Your desired rate is <b>", rate_calc_desired_rate(), " breaths/min</b>.", sep = "")
  out_rate_limit_warning <- case_when(
    rate_calc_desired_rate() > 28 ~ paste("<br><br><span style=\"color:#d9534f\">WARNING: at this rate, a time constant calculation is recommended to prevent air trapping.</span>"),
    rate_calc_desired_rate() < 10 ~ paste("<br><br><span style=\"color:#d9534f\">WARNING: this is a very low rate, and may lead to progressive atelectasis.</span>"),
    TRUE ~ paste("")
  )
  return(paste(out_result, out_rate_limit_warning))
})
...
renderCachedPlot(
  {
    ...
    desired_rate <- rate_calc_desired_rate()
  ...
  cacheKeyExpr = list(input$current_pco2, input$current_rate, input$desired_pco2)
)
```
...
observeEvent(input$rv_current_pco2, {
  if (!is.numeric(input$rv_current_pco2)) {
    shinyFeedback::feedbackDanger(
      "rv_current_pco2",
      !is.numeric(input$rv_current_pco2),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$rv_current_pco2 <= 0) {
    shinyFeedback::feedbackDanger(
      "rv_current_pco2",
      input$rv_current_pco2 <= 0,
      "This calculator does not support negative values.",
      icon = NULL
    )
  }
  else if (input$rv_current_pco2 < 20 | input$rv_current_pco2 > 60) {
    shinyFeedback::feedbackDanger(
      "rv_current_pco2",
      input$rv_current_pco2 < 20 | input$rv_current_pco2 > 60,
      "This is a potentially dangerous CO2 value. Are you sure?",
      icon = NULL
    )
  }
  else if (!is.integer(input$rv_current_pco2)) {
    shinyFeedback::feedbackWarning(
      "rv_current_pco2",
      !is.integer(input$rv_current_pco2),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("rv_current_pco2")
  }
})
...
observeEvent(input$rv_current_rate, {
  if (!is.numeric(input$rv_current_rate)) {
    shinyFeedback::feedbackDanger(
      "rv_current_rate",
      !is.numeric(input$rv_current_rate),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$rv_current_rate <= 0) {
    shinyFeedback::feedbackDanger(
      "rv_current_rate",
      input$rv_current_rate <= 0,
      "This calculator does not support negative values.",
      icon = NULL
    )
  }
  else if (input$rv_current_rate < 10 | input$rv_current_rate > 40) {
    shinyFeedback::feedbackDanger(
      "rv_current_rate",
      input$rv_current_rate < 10 | input$rv_current_rate > 40,
      "This is a potentially dangerous rate. Are you sure?",
      icon = NULL
    )
  }
  else if (!is.integer(input$rv_current_rate)) {
    shinyFeedback::feedbackWarning(
      "rv_current_rate",
      !is.integer(input$rv_current_rate),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("rv_current_rate")
  }
})
...
observeEvent(input$rv_current_tv, {
  if (!is.numeric(input$rv_current_tv)) {
    shinyFeedback::feedbackDanger(
      "rv_current_tv",
      !is.numeric(input$rv_current_tv),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$rv_current_tv <= 0) {
    shinyFeedback::feedbackDanger(
      "rv_current_tv",
      input$rv_current_tv <= 0,
      "This calculator does not support negative values.",
      icon = NULL
    )
  }
  else if (input$rv_current_tv < 4 | input$rv_current_tv > 10) {
    shinyFeedback::feedbackDanger(
      "rv_current_tv",
      input$rv_current_tv < 4 | input$rv_current_tv > 10,
      "This is a potentially dangerous tidal volume. Are you sure?",
      icon = NULL
    )
  }
  else if (!is.integer(input$rv_current_tv)) {
    shinyFeedback::feedbackWarning(
      "rv_current_tv",
      !is.integer(input$rv_current_tv),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("rv_current_tv")
  }
})
...
observeEvent(input$rv_desired_pco2, {
  if (!is.numeric(input$rv_desired_pco2)) {
    shinyFeedback::feedbackDanger(
      "rv_desired_pco2",
      !is.numeric(input$rv_desired_pco2),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$rv_desired_pco2 <= 0) {
    shinyFeedback::feedbackDanger(
      "rv_desired_pco2",
      input$rv_desired_pco2 <= 0,
      "This calculator does not support negative values.",
      icon = NULL
    )
  }
  else if (input$rv_desired_pco2 < 20 | input$rv_desired_pco2 > 60) {
    shinyFeedback::feedbackDanger(
      "rv_desired_pco2",
      input$rv_desired_pco2 < 20 | input$rv_desired_pco2 > 60,
      "This is a potentially dangerous CO2 value. Are you sure?",
      icon = NULL
    )
  }
  else if (!is.integer(input$rv_desired_pco2)) {
    shinyFeedback::feedbackWarning(
      "rv_desired_pco2",
      !is.integer(input$rv_desired_pco2),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("rv_desired_pco2")
  }
})
...
observeEvent(c(input$rv_desired_set, input$rv_desired_numeric), {
  if (!is.numeric(input$rv_desired_numeric)) {
    shinyFeedback::feedbackDanger(
      "rv_desired_numeric",
      !is.numeric(input$rv_desired_numeric),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$rv_desired_numeric <= 0) {
    shinyFeedback::feedbackDanger(
      "rv_desired_numeric",
      input$rv_desired_numeric <= 0,
      "This calculator does not support negative values.",
      icon = NULL
    )
  }
  else if (input$rv_desired_set == "Set Tidal Volume" & input$rv_desired_numeric < 4) {
    shinyFeedback::feedbackDanger(
      "rv_desired_numeric",
      input$rv_desired_numeric < 4 | input$rv_desired_numeric > 10,
      "This is a potentially dangerous tidal volume. Are you sure?",
      icon = NULL
    )
  }
  else if (input$rv_desired_set == "Set Tidal Volume" & input$rv_desired_numeric > 10) {
    shinyFeedback::feedbackDanger(
      "rv_desired_numeric",
      input$rv_desired_numeric < 4 | input$rv_desired_numeric > 10,
      "This is a potentially dangerous tidal volume. Are you sure?",
  ...
  else if (input$rv_desired_set == "Set Rate" & input$rv_desired_numeric < 10) {
    shinyFeedback::feedbackDanger(
      "rv_desired_numeric",
      input$rv_desired_numeric < 10 | input$rv_desired_numeric > 40,
      "This is a potentially dangerous rate. Are you sure?",
      icon = NULL
    )
  }
  else if (input$rv_desired_set == "Set Rate" & input$rv_desired_numeric > 40) {
    shinyFeedback::feedbackDanger(
      "rv_desired_numeric",
      input$rv_desired_numeric < 10 | input$rv_desired_numeric > 40,
      "This is a potentially dangerous rate. Are you sure?",
  ...
  else if (!is.integer(input$rv_desired_numeric)) {
    shinyFeedback::feedbackWarning(
      "rv_desired_numeric",
      !is.integer(input$rv_desired_numeric),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("rv_desired_numeric")
  }
})
```
...
output$rv_numeric <- renderText({
  validate(
    need(input$rv_current_pco2 > 0, "This calculator only accepts values greater than zero. Please enter a current pCO2 value greater than zero."),
    need(input$rv_current_rate > 0, "This calculator only accepts values greater than zero. Please enter a current rate value greater than zero."),
    need(input$rv_current_tv > 0, "This calculator only accepts values greater than zero. Please enter a current tidal volume value greater than zero."),
    need(input$rv_desired_pco2 > 0, "This calculator only accepts values greater than zero. Please enter a desired pCO2 volume value greater than zero."),
    need(input$rv_desired_numeric > 0, "This calculator only accepts values greater than zero. Please enter a desired value greater than zero.")
  )
  ...
  warning_vt <- case_when(
    desired_vt_rv > 10 ~ paste("<br><br><span style=\"color:#d9534f\">WARNING: your target tidal volume exceeds 10 ml/kg. This tidal volume may be associated with volutrauma. Please ensure there are no signs of overdistention on pressure-volume loops.</span>"),
    desired_vt_rv < 4 ~ paste("<br><br><span style=\"color:#d9534f\">WARNING: your target tidal volume is less than 4 ml/kg. This tidal volume is low, and may be associated with progressive atelectasis. Please ensure this is not an error.</span>"),
    desired_vt_rv >= 4 & desired_vt_rv <= 10 ~ paste("")
  )
  warning_rate <- case_when(
    desired_rate_rv > 28 ~ paste("<br><br><span style=\"color:#d9534f\">WARNING: your target rate exceeds 28 breaths/min. At this rate, a time constant calculation is recommended to prevent air trapping.</span>"),
    desired_rate_rv < 10 ~ paste("<br><br><span style=\"color:#d9534f\">WARNING: your target rate is less than 10 breaths/min. This rate is low, and may be associated with progressive atelectasis. Please ensure this is not an error.</span>"),
    desired_rate_rv >= 10 & desired_rate_rv <= 28 ~ paste("")
  )
  return(paste(rv_result, warning_vt, warning_rate))
})
...
renderCachedPlot(
  ...
  cacheKeyExpr = list(input$rv_current_rate, input$rv_current_tv, input$rv_current_pco2, input$rv_desired_pco2)
)
```
...
renderCachedPlot(
  ...
  cacheKeyExpr = list(input$rv_current_pco2, input$rv_current_rate, input$rv_current_tv, input$rv_desired_pco2, input$rv_desired_numeric)
)
```
...
observeEvent(input$cr_vt, {
  if (!is.numeric(input$cr_vt)) {
    shinyFeedback::feedbackDanger(
      "cr_vt",
      !is.numeric(input$cr_vt),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$cr_vt <= 0) {
    shinyFeedback::feedbackDanger(
      "cr_vt",
      input$cr_vt <= 0,
      "This calculator does not support negative values.",
  ...
  else if (input$cr_vt < 10 | input$cr_vt > 700) {
    shinyFeedback::feedbackDanger(
      "cr_vt",
      input$cr_vt < 10 | input$cr_vt > 700,
      "This is a potentially dangerous tidal volume. Are you sure?",
      icon = NULL
    )
  }
  else if (!is.integer(input$cr_vt)) {
    shinyFeedback::feedbackWarning(
      "cr_vt",
      !is.integer(input$cr_vt),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("cr_vt")
  }
})
...
observeEvent(input$cr_pip, {
  if (!is.numeric(input$cr_pip)) {
    shinyFeedback::feedbackDanger(
      "cr_pip",
      !is.numeric(input$cr_pip),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$cr_pip <= 0) {
    shinyFeedback::feedbackDanger(
      "cr_pip",
      input$cr_pip <= 0,
      "This calculator does not support negative values.",
  ...
  else if (input$cr_pip < 8 | input$cr_pip > 35) {
    shinyFeedback::feedbackDanger(
      "cr_pip",
      input$cr_pip < 8 | input$cr_pip > 35,
      "This is a potentially dangerous peak pressure. Are you sure?",
      icon = NULL
    )
  }
  else if (!is.integer(input$cr_pip)) {
    shinyFeedback::feedbackWarning(
      "cr_pip",
      !is.integer(input$cr_pip),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("cr_pip")
  }
})
...
observeEvent(input$cr_pplat, {
  if (!is.numeric(input$cr_pplat)) {
    shinyFeedback::feedbackDanger(
      "cr_pplat",
      !is.numeric(input$cr_pplat),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$cr_pplat <= 0) {
    shinyFeedback::feedbackDanger(
      "cr_pplat",
      input$cr_pplat <= 0,
      "This calculator does not support negative values.",
  ...
  else if (input$cr_pplat < 8 | input$cr_pplat > 35) {
    shinyFeedback::feedbackDanger(
      "cr_pplat",
      input$cr_pplat < 8 | input$cr_pplat > 35,
      "This is a potentially dangerous plateau pressure. Are you sure?",
      icon = NULL
    )
  }
  else if (!is.integer(input$cr_pplat)) {
    shinyFeedback::feedbackWarning(
      "cr_pplat",
      !is.integer(input$cr_pplat),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("cr_pplat")
  }
})
...
observeEvent(input$cr_peep, {
  if (!is.numeric(input$cr_peep)) {
    shinyFeedback::feedbackDanger(
      "cr_peep",
      !is.numeric(input$cr_peep),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$cr_peep <= 0) {
    shinyFeedback::feedbackDanger(
      "cr_peep",
      input$cr_peep <= 0,
      "This calculator does not support negative values.",
  ...
  else if (input$cr_peep < 5 | input$cr_peep > 20) {
    shinyFeedback::feedbackDanger(
      "cr_peep",
      input$cr_peep < 5 | input$cr_peep > 20,
      "This is a potentially dangerous PEEP value. Are you sure?",
      icon = NULL
    )
  }
  else if (!is.integer(input$cr_peep)) {
    shinyFeedback::feedbackWarning(
      "cr_peep",
      !is.integer(input$cr_peep),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("cr_peep")
  }
})
...
observeEvent(input$cr_itime, {
  if (!is.numeric(input$cr_itime)) {
    shinyFeedback::feedbackDanger(
      "cr_itime",
      !is.numeric(input$cr_itime),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$cr_itime <= 0) {
    shinyFeedback::feedbackDanger(
      "cr_itime",
      input$cr_itime <= 0,
      "This calculator does not support negative values.",
  ...
  else if (input$cr_itime < 8 | input$cr_itime > 35) {
    shinyFeedback::feedbackDanger(
      "cr_itime",
      input$cr_itime < 0.4 | input$cr_itime > 2,
      "This is a potentially dangerous iTime. Are you sure?",
      icon = NULL
    )
  }
  else {
    shinyFeedback::hideFeedback("cr_itime")
  }
})
...
observeEvent(input$cr_height, {
  if (!is.numeric(input$cr_height)) {
    shinyFeedback::feedbackDanger(
      "cr_height",
      !is.numeric(input$cr_height),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$cr_height <= 0) {
    shinyFeedback::feedbackDanger(
      "cr_height",
      input$cr_height <= 0,
      "This calculator does not support negative values.",
  ...
  else if (input$cr_height < 40) {
    shinyFeedback::feedbackDanger(
      "cr_height",
      input$cr_height < 40,
      "The average height of a term newborn is 50 cm. Are you sure?",
      icon = NULL
    )
  }
  else if (input$cr_height > 200) {
    shinyFeedback::feedbackDanger(
      "cr_height",
      input$cr_height > 200,
      "The average height of adult males and females are 175 cm and 162 cm, respectively. Are you sure?",
      icon = NULL
    )
  }
  else {
    shinyFeedback::hideFeedback("cr_height")
  }
})
```
...
renderTable({
  validate(
    need(input$cr_pplat > 0, "Please enter a numeric plateau pressure greater than zero."),
    need(input$cr_peep > 0, "Please enter a numeric positive end expiratory pressure greater than zero."),
    need(input$cr_vt > 0, "Please enter a numeric tidal volume greater than zero."),
    need(input$cr_pip > 0, "Please enter a numeric peak pressure greater than zero."),
    need(input$cr_itime > 0, "Please enter a numeric inspiratory time greater than zero."),
    need(input$cr_pip > input$cr_peep, "Please enter a PIP greater than PEEP."),
    need(input$cr_pplat > input$cr_peep, "Please enter a plateau pressure greater than PEEP."),
    need(input$cr_pip > input$cr_pplat, "Please enter a PIP greater than plateau pressure.")
  )
...
renderCachedPlot(
  {
    validate(
      ...
      need(input$cr_pplat > input$cr_peep, "Please enter a plateau pressure greater than PEEP."),
      need(input$cr_pip > input$cr_pplat, "Please enter a PIP greater than plateau pressure."),
      need(input$cr_height >= 40 & input$cr_height <= 200, "Please enter a height between 40 and 200 cm.")
    )
  ...
  cacheKeyExpr = list(input$cr_vt, input$cr_pplat, input$cr_peep, input$cr_height)
)
```
...
renderCachedPlot(
  ...
  cacheKeyExpr = list(input$cr_height, input$cr_vt, input$cr_pplat, input$cr_peep)
)
```
...
observeEvent(input$tc_v_1, {
  if (!is.numeric(input$tc_v_1)) {
    shinyFeedback::feedbackDanger(
      "tc_v_1",
      !is.numeric(input$tc_v_1),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$tc_v_1 <= 0) {
    shinyFeedback::feedbackDanger(
      "tc_v_1",
      input$tc_v_1 <= 0,
      "This calculator does not support negative values.",
  ...
  else if (!is.integer(input$tc_v_1)) {
    shinyFeedback::feedbackWarning(
      "tc_v_1",
      !is.integer(input$tc_v_1),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("tc_v_1")
  }
})
...
observeEvent(input$tc_flow_1, {
  if (!is.numeric(input$tc_flow_1)) {
    shinyFeedback::feedbackDanger(
      "tc_flow_1",
      !is.numeric(input$tc_flow_1),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$tc_flow_1 <= 0) {
    shinyFeedback::feedbackDanger(
      "tc_flow_1",
      input$tc_flow_1 <= 0,
      "This calculator does not support negative values.",
  ...
  else if (!is.integer(input$tc_flow_1)) {
    shinyFeedback::feedbackWarning(
      "tc_flow_1",
      !is.integer(input$tc_flow_1),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("tc_flow_1")
  }
})
...
observeEvent(input$tc_v_2, {
  if (!is.numeric(input$tc_v_2)) {
    shinyFeedback::feedbackDanger(
      "tc_v_2",
      !is.numeric(input$tc_v_2),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$tc_v_2 <= 0) {
    shinyFeedback::feedbackDanger(
      "tc_v_2",
      input$tc_v_2 <= 0,
      "This calculator does not support negative values.",
  ...
  else if (!is.integer(input$tc_v_2)) {
    shinyFeedback::feedbackWarning(
      "tc_v_2",
      !is.integer(input$tc_v_2),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("tc_v_2")
  }
})
...
observeEvent(input$tc_flow_2, {
  if (!is.numeric(input$tc_flow_2)) {
    shinyFeedback::feedbackDanger(
      "tc_flow_2",
      !is.numeric(input$tc_flow_2),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$tc_flow_2 <= 0) {
    shinyFeedback::feedbackDanger(
      "tc_flow_2",
      input$tc_flow_2 <= 0,
      "This calculator does not support negative values.",
  ...
  else if (!is.integer(input$tc_flow_2)) {
    shinyFeedback::feedbackWarning(
      "tc_flow_2",
      !is.integer(input$tc_flow_2),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("tc_flow_2")
  }
})
...
observeEvent(input$tc_v_3, {
  if (!is.numeric(input$tc_v_3)) {
    shinyFeedback::feedbackDanger(
      "tc_v_3",
      !is.numeric(input$tc_v_3),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$tc_v_3 <= 0) {
    shinyFeedback::feedbackDanger(
      "tc_v_3",
      input$tc_v_3 <= 0,
      "This calculator does not support negative values.",
  ...
  else if (!is.integer(input$tc_v_3)) {
    shinyFeedback::feedbackWarning(
      "tc_v_3",
      !is.integer(input$tc_v_3),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("tc_v_3")
  }
})
...
observeEvent(input$tc_flow_3, {
  if (!is.numeric(input$tc_flow_3)) {
    shinyFeedback::feedbackDanger(
      "tc_flow_3",
      !is.numeric(input$tc_flow_3),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$tc_flow_3 <= 0) {
    shinyFeedback::feedbackDanger(
      "tc_flow_3",
      input$tc_flow_3 <= 0,
      "This calculator does not support negative values.",
  ...
  else if (!is.integer(input$tc_flow_3)) {
    shinyFeedback::feedbackWarning(
      "tc_flow_3",
      !is.integer(input$tc_flow_3),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("tc_flow_3")
  }
})
...
observeEvent(input$tc_itime, {
  if (!is.numeric(input$tc_itime)) {
    shinyFeedback::feedbackDanger(
      "tc_itime",
      !is.numeric(input$tc_itime),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$tc_itime <= 0) {
    shinyFeedback::feedbackDanger(
      "tc_itime",
      input$tc_itime <= 0,
      "This calculator does not support negative values.",
  ...
  else if (input$tc_itime < 8 | input$tc_itime > 35) {
    shinyFeedback::feedbackDanger(
      "tc_itime",
      input$tc_itime < 0.4 | input$tc_itime > 2,
      "This is a potentially dangerous iTime. Are you sure?",
      icon = NULL
    )
  }
  else {
    shinyFeedback::hideFeedback("tc_itime")
  }
})
```
...
Below is the patient's calculated _expiratory_ time constant and maximum safe respiratory rate to prevent air trapping. This calculator will use linear regression to average 3 readings, to minimize error caused by rounding on the ventilator. **However**, the regression line should have a near perfect correlation. If the $r^{2}$ value is not at least 0.9, please consider repeating your measurements.

```{r tc_warning}
output$tc_warning <- renderText({
  validate(
    need(input$tc_v_1 > 0, "Please enter a Volume #1 greater than zero."),
    need(input$tc_flow_1 > 0, "Please enter a Flow #1 greater than zero."),
    need(input$tc_v_2 > 0, "Please enter a Volume #2 greater than zero."),
    need(input$tc_flow_2 > 0, "Please enter a Flow #2 greater than zero."),
    need(input$tc_v_3 > 0, "Please enter a Volume #3 greater than zero."),
    need(input$tc_flow_3 > 0, "Please enter a Flow #3 greater than zero."),
    need(input$tc_itime > 0, "Please enter an inspiratoy time greater than zero.")
  )
  vol_1 <- input$tc_v_1
  ...
  if (tc_regression_r_squared < 1 & tc_regression_r_squared > 0.9) {
    out <- paste("<br>The regression adjusted r^2 value is: ", round(tc_regression_r_squared, 3), ". This is likely acceptable.", sep = "")
  }
  if (tc_regression_r_squared < 0.9) {
    out <- paste("<br><span style=\"color:#d9534f\">WARNING: the regression adjusted r^2 value is: ", round(tc_regression_r_squared, 3), ". This suggests a variable expiratory flow waveform. Possible explantions include secretions, active exhalation, or dynamic airway collapse.</span>", sep = "")
  }
  return(paste(out))
})
...
Flow is negative (away from the patient) during exhalation. Thus, the negative signs cancel out, resulting in a positive value for $\tau$. However, because most ventilators do not display flow as a negative number, VentmindeR is designed to take positive numbers for both flow and volume (exactly as you see them on the ventilator).
...
renderCachedPlot(
  ...
  cacheKeyExpr = list(input$tc_v_1, input$tc_flow_1, input$tc_v_2, input$tc_flow_2, input$tc_v_3, input$tc_flow_3, input$tc_itime)
)
```
...
renderCachedPlot(
  ...
  cacheKeyExpr = list(input$tc_itime, input$tc_v_1, input$tc_flow_1, input$tc_v_2, input$tc_flow_2, input$tc_v_3, input$tc_flow_3)
)
```
...
renderCachedPlot(
  ...
  cacheKeyExpr = list(input$tc_flow_3, input$tc_itime, input$tc_v_1, input$tc_flow_1, input$tc_v_2, input$tc_flow_2, input$tc_v_3)
)
```
...
observeEvent(input$vdvt_paco2, {
  if (!is.numeric(input$vdvt_paco2)) {
    shinyFeedback::feedbackDanger(
      "vdvt_paco2",
      !is.numeric(input$vdvt_paco2),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$vdvt_paco2 <= 0) {
    shinyFeedback::feedbackDanger(
      "vdvt_paco2",
      input$vdvt_paco2 <= 0,
      "This calculator does not support negative values.",
  ...
  else if (input$vdvt_paco2 < 20 | input$vdvt_paco2 > 60) {
    shinyFeedback::feedbackDanger(
      "vdvt_paco2",
      input$vdvt_paco2 < 20 | input$vdvt_paco2 > 60,
      "This is a potentially dangerous CO2 value. Are you sure?",
      icon = NULL
    )
  }
  else if (!is.integer(input$vdvt_paco2)) {
    shinyFeedback::feedbackWarning(
      "vdvt_paco2",
      !is.integer(input$vdvt_paco2),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("vdvt_paco2")
  }
})
...
observeEvent(input$vdvt_peco2, {
  if (!is.numeric(input$vdvt_peco2)) {
    shinyFeedback::feedbackDanger(
      "vdvt_peco2",
      !is.numeric(input$vdvt_peco2),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$vdvt_peco2 <= 0) {
    shinyFeedback::feedbackDanger(
      "vdvt_peco2",
      input$vdvt_peco2 <= 0,
      "This calculator does not support negative values.",
  ...
  else if (input$vdvt_peco2 < 20 | input$vdvt_peco2 > 60) {
    shinyFeedback::feedbackDanger(
      "vdvt_peco2",
      input$vdvt_peco2 < 20 | input$vdvt_peco2 > 60,
      "This is a potentially dangerous CO2 value. Are you sure?",
      icon = NULL
    )
  }
  else if (!is.integer(input$vdvt_peco2)) {
    shinyFeedback::feedbackWarning(
      "vdvt_peco2",
      !is.integer(input$vdvt_peco2),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("vdvt_peco2")
  }
})
```
...
output$vd_vt <- renderText({
  validate(
    need(input$vdvt_paco2 > 0, "Please enter a numeric PaCO2 value greater than zero."),
    need(input$vdvt_peco2 > 0, "Please enter a numeric End Tidal CO2 value greater than zero."),
    need(input$vdvt_paco2 > input$vdvt_peco2, "Please enter a PaCO2 value greater than End Tidal CO2 You entered a PaCO2 less than End Tidal CO2. While this is possible in special circumstances (Karnik et. al.), it is not possible to calculate Vd/Vt. Did you reverse PaCO2 and End Tidal CO2?")
  )
  vd_vt <- (input$vdvt_paco2 - input$vdvt_peco2) / input$vdvt_paco2
  vd_vt_numeric <- paste("The patient's Vd/Vt is: <b>", round(vd_vt, 2), "</b>.<br><br>Normal Vd/Vt in children was originally found to be approximately 0.33 ± 0.05 in the original 1976 Kerr study, but has been suggested to be lower in more recent studies.", sep = "")

  return(paste(vd_vt_numeric))
})
...
observeEvent(input$oi_fio2, {
  if (!is.numeric(input$oi_fio2)) {
    shinyFeedback::feedbackDanger(
      "oi_fio2",
      !is.numeric(input$oi_fio2),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$oi_fio2 <= 0) {
    shinyFeedback::feedbackDanger(
      "oi_fio2",
      input$oi_fio2 <= 0,
      "This calculator does not support negative values.",
  ...
  else if (input$oi_fio2 < 21) {
    shinyFeedback::feedbackDanger(
      "oi_fio2",
      input$oi_fio2 < 21,
      "The FiO2 of room air is 21%. Is your patient really on hypoxic mix?",
      icon = NULL
    )
  }
  else if (input$oi_fio2 > 100) {
    shinyFeedback::feedbackDanger(
      "oi_fio2",
      input$oi_fio2 > 100,
      "Please enter an FiO2 ≤ 100%.",
      icon = NULL
    )
  }
  else if (!is.integer(input$oi_fio2)) {
    shinyFeedback::feedbackWarning(
      "oi_fio2",
      !is.integer(input$oi_fio2),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("oi_fio2")
  }
})
...
observeEvent(input$oi_map, {
  if (!is.numeric(input$oi_map)) {
    shinyFeedback::feedbackDanger(
      "oi_map",
      !is.numeric(input$oi_map),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$oi_map <= 0) {
    shinyFeedback::feedbackDanger(
      "oi_map",
      input$oi_map <= 0,
      "This calculator does not support negative values.",
  ...
  else if (input$oi_map < 5 | input$oi_map > 20) {
    shinyFeedback::feedbackDanger(
      "oi_map",
      input$oi_map < 5 | input$oi_map > 30,
      "This is a potentially dangerous MAP value. Are you sure?",
      icon = NULL
    )
  }
  else if (!is.integer(input$oi_map)) {
    shinyFeedback::feedbackWarning(
      "oi_map",
      !is.integer(input$oi_map),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("oi_map")
  }
})
...
observeEvent(input$oi_peep, {
  if (!is.numeric(input$oi_peep)) {
    shinyFeedback::feedbackDanger(
      "oi_peep",
      !is.numeric(input$oi_peep),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$oi_peep <= 0) {
    shinyFeedback::feedbackDanger(
      "oi_peep",
      input$oi_peep <= 0,
      "This calculator does not support negative values.",
  ...
  else if (input$oi_peep < 5 | input$oi_peep > 20) {
    shinyFeedback::feedbackDanger(
      "oi_peep",
      input$oi_peep < 5 | input$oi_peep > 20,
      "This is a potentially dangerous PEEP value. Are you sure?",
      icon = NULL
    )
  }
  else if (!is.integer(input$oi_peep)) {
    shinyFeedback::feedbackWarning(
      "oi_peep",
      !is.integer(input$oi_peep),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("oi_peep")
  }
})
...
oi_pao2_reactive <- reactive(list(input$oi_desired, input$oi_pao2))

observeEvent(oi_pao2_reactive(), {
  if (!is.numeric(input$oi_pao2)) {
    shinyFeedback::feedbackDanger(
      "oi_pao2",
      !is.numeric(input$oi_pao2),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$oi_pao2 <= 0) {
    shinyFeedback::feedbackDanger(
      "oi_pao2",
      input$oi_pao2 <= 0,
      "This calculator does not support negative values.",
  ...
  else if (input$oi_desired == "S/F and OSI" & input$oi_pao2 >= 100) {
    shinyFeedback::feedbackDanger(
      "oi_pao2",
      input$oi_desired == "S/F and OSI" & input$oi_pao2 >= 100,
      "Please enter an SpO2 < 100%. Did you mean to enter a PaO2 (mmHg)?",
      icon = NULL
    )
  }
  else if (!is.integer(input$oi_pao2)) {
    shinyFeedback::feedbackWarning(
      "oi_pao2",
      !is.integer(input$oi_pao2),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("oi_pao2")
  }
})
```
...
output$pf_oi_calculation <- renderText({
  validate(
    need(input$oi_pao2 > 0, "Please enter a numeric PaO2 (mmHg) or SpO2 (%) value greater than zero."),
    need(input$oi_fio2 > 0, "Please enter a numeric FiO2 (%) value greater than zero."),
    need(input$oi_map > 0, "Please enter a numeric mean airway pressure (cm H2O) value greater than zero.")
  )
  ...
  numeric_calculation <- case_when(
    ...
    input$oi_desired == "S/F and OSI" & input$oi_pao2 > 100 ~ paste("<span style=\"color:#d9534f\">You have selected an oxygen saturation of >100%. This is not posible. Did you mean to enter a PaO2?</span>"),
    input$oi_desired == "S/F and OSI" & input$oi_pao2 == 100 ~ paste("<span style=\"color:#d9534f\">You have selected an oxygen saturation of 100%. Is this actually your patient's oxygen saturation? If so, you should not attempt to calculate an an S/F ratio or OSI. It is impossible to infer a PaO2 from an SpO2 of 100% (e.g. both a PaO2 of 150 mmHg and 450 mmHg will result in an SpO2 of 100%). Did you mean to enter a PaO2?</span>"),
  ...
  return(paste(numeric_calculation))
})
...
renderCachedPlot(
  {
    validate(
      need(input$oi_fio2 > 0, "Please enter a numeric FiO2 (%) value greater than zero."),
      need(input$oi_peep > 0, "Please enter a numeric PEEP (cm H2O) value greater than zero.")
    )
  ...
  cacheKeyExpr = list(input$oi_fio2, input$oi_peep)
)
```
...
renderCachedPlot(
  ...
  cacheKeyExpr = list(input$oi_peep, input$oi_fio2)
)
```
...
observeEvent(input$ao_sat, {
  if (!is.numeric(input$ao_sat)) {
    shinyFeedback::feedbackDanger(
      "ao_sat",
      !is.numeric(input$ao_sat),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$ao_sat < 0) {
    shinyFeedback::feedbackDanger(
      "ao_sat",
      input$ao_sat < 0,
      "This calculator does not support negative values.",
  ...
  else if (input$ao_sat < 60) {
    shinyFeedback::feedbackDanger(
      "ao_sat",
      input$ao_sat < 60,
      "This is a potentially dangerous arterial oxgyen saturation (%) value. Are you sure?",
      icon = NULL
    )
  }
  else if (!is.integer(input$ao_sat)) {
    shinyFeedback::feedbackWarning(
      "ao_sat",
      !is.integer(input$ao_sat),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("ao_sat")
  }
})
...
observeEvent(input$svc_sat, {
  if (!is.numeric(input$svc_sat)) {
    shinyFeedback::feedbackDanger(
      "svc_sat",
      !is.numeric(input$svc_sat),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$svc_sat < 0) {
    shinyFeedback::feedbackDanger(
      "svc_sat",
      input$svc_sat < 0,
      "This calculator does not support negative values.",
  ...
  else if (input$svc_sat > 80) {
    shinyFeedback::feedbackDanger(
      "svc_sat",
      input$svc_sat > 80,
      "This is a high mixed venous saturation (%) value. Are you sure?",
      icon = NULL
    )
  }
  else if (input$svc_sat < 40) {
    shinyFeedback::feedbackDanger(
      "svc_sat",
      input$svc_sat < 40,
      "This is a potentially dangeously low mixed venous saturation (%) value. Are you sure?",
      icon = NULL
    )
  }
  else if (!is.integer(input$svc_sat)) {
    shinyFeedback::feedbackWarning(
      "svc_sat",
      !is.integer(input$svc_sat),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("svc_sat")
  }
})
...
observeEvent(input$pv_sat, {
  if (!is.numeric(input$pv_sat)) {
    shinyFeedback::feedbackDanger(
      "pv_sat",
      !is.numeric(input$pv_sat),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$pv_sat < 0) {
    shinyFeedback::feedbackDanger(
      "pv_sat",
      input$pv_sat < 0,
      "This calculator does not support negative values.",
  ...
  else if (input$pv_sat < 90) {
    shinyFeedback::feedbackDanger(
      "pv_sat",
      input$pv_sat < 90,
      "This is a surprisingly low pulmonary venous oxygen saturation (%) value. Are you sure?",
      icon = NULL
    )
  }
  else if (!is.integer(input$pv_sat)) {
    shinyFeedback::feedbackWarning(
      "pv_sat",
      !is.integer(input$pv_sat),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("pv_sat")
  }
})
...
observeEvent(input$hgb, {
  if (!is.numeric(input$hgb)) {
    shinyFeedback::feedbackDanger(
      "hgb",
      !is.numeric(input$hgb),
      "A numeric value is required.",
      icon = NULL
    )
  }
  ...
  else if (input$hgb > 20) {
    shinyFeedback::feedbackDanger(
      "hgb",
      input$hgb > 20,
      "This is a surprisingly high hemoglobin (mg/dL). Are you sure?",
      icon = NULL
    )
  }
  else if (input$hgb < 7) {
    shinyFeedback::feedbackDanger(
      "hgb",
      input$hgb < 7,
      "This is a potentially dangerously low hemoglobin (mg/dL). Are you sure?",
      icon = NULL
    )
  }
  else if (!is.integer(input$hgb)) {
    shinyFeedback::feedbackWarning(
      "hgb",
      !is.integer(input$hgb),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("hgb")
  }
})
...
observeEvent(input$qp_qs_pao2, {
  if (!is.numeric(input$qp_qs_pao2)) {
    shinyFeedback::feedbackDanger(
      "qp_qs_pao2",
      !is.numeric(input$qp_qs_pao2),
      "A numeric value is required.",
      icon = NULL
    )
  }
  else if (input$qp_qs_pao2 < 0) {
    shinyFeedback::feedbackDanger(
      "qp_qs_pao2",
      input$qp_qs_pao2 < 0,
      "This calculator does not support negative values.",
  ...
  else if (input$qp_qs_pao2 > 200) {
    shinyFeedback::feedbackDanger(
      "qp_qs_pao2",
      input$qp_qs_pao2 > 200,
      "This is a surprisingly high PaO2 (mmHg). Are you sure?",
      icon = NULL
    )
  }
  else if (input$qp_qs_pao2 < 30) {
    shinyFeedback::feedbackDanger(
      "qp_qs_pao2",
      input$qp_qs_pao2 < 30,
      "This is a potentially dangerously low PaO2 (mmHg). Are you sure?",
      icon = NULL
    )
  }
  else if (!is.integer(input$qp_qs_pao2)) {
    shinyFeedback::feedbackWarning(
      "qp_qs_pao2",
      !is.integer(input$qp_qs_pao2),
      "This isn't an integer. Are you sure?",
      color = "#FF7F24",
      icon = NULL
    )
    ...
    shinyFeedback::hideFeedback("qp_qs_pao2")
  }
})
```
...
output$qp_qs_result <- renderText({
  validate(
    need(input$ao_sat > 0 & input$ao_sat <= 100, "Please enter a numeric arterial oxygen saturation value between 0 and 100%."),
    need(input$pv_sat > 0 & input$pv_sat <= 100, "Please enter a numeric pulmonary venous oxygen saturation value between 0 and 100%."),
    need(input$svc_sat > 0 & input$svc_sat <= 100, "Please enter a numeric mixed venous oxygen saturation value between 0 and 100%."),
    need(input$pv_sat > input$ao_sat, "Please enter a pulmonary venous oxygen saturation (%) greater than the arterial oxygen saturation (%)."),
    need(input$ao_sat > input$svc_sat, "Please enter a arterial oxygen saturation (%) greater than the mixed venous oxygen saturation (%)."),
    need(input$hgb > 0, "Please enter a numeric hemoglobin (mg/dL) value greater than zero.")
  )
...
renderCachedPlot(
  ...
  cacheKeyExpr = list(input$svc_sat, input$ao_sat, input$pv_sat)
)
```
...
renderCachedPlot(
  {
    validate(
      need(input$ao_sat > 0 & input$ao_sat <= 100, "Please enter a numeric arterial oxygen saturation value between 0 and 100%."),
      need(input$qp_qs_pao2 > 0 & input$qp_qs_pao2 <= 150, "Please enter a numeric PaO2 (mmHg) value between 0 mmHg and 150 mmHg.")
    )
  ...
  cacheKeyExpr = list(input$qp_qs_pao2, input$ao_sat)
)
```
...
Thanks to all of the beta-testers for VentmindeR, the instructors of the University of Pittsburgh Medical Center Critical Care Medicine ventilator course, and all of our research mentors. Special thanks to Dr. Christopher Horvat for mentoring and Dr. Patrick Kochanek for sponsoring the T32 that gave us time to work on this project.
...
[@RN1; @RN11; @RN12; @RN14; @RN15; @RN16; @RN17; @RN18; @RN19; @RN2; @RN20; @RN21; @RN22; @RN23; @RN24; @RN25; @RN26; @RN27; @RN28; @RN29; @RN3; @RN30; @RN31; @RN32; @RN4; @RN5; @RN6; @RN8]
